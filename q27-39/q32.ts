...
console.log("\n \t questionno 32")
let current_users : string[] = ["eric", "peter","robbert","harry","micale"];
let new_users: string[] = ["sara","Maria","Loren","Harry","Micale"]
new_users.forEach((new_user) =>{if (current_users.some((current_user) =>
     current_user.toLowerCase()===new_user.toLowerCase()
     )
     )
     {console.log(`${new_user} already exist ,please try another name`);}
    else {console.log (`${new_user} you can continue`)}})
